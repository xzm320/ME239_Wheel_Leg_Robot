...
100 km/h 目标把左右轮距从 227 mm 加到 4 倍（910 mm），并在 32 m 宽、3000 m 长的赛道上用速度 PI + 俯仰 PD、0.70 m/s² 加速。峰值达到 27.94 m/s（100.6 km/h），驶过约 1001 m（含进入崎岖段后的高速段）；约 62.9 s 后航向发散冲出路肩，横滚超过 15°，因此还不是完整的 64 s 巡航。1x–3.5x 轮距无法同时把横滚压住并跑到 100 km/h。高速尝试额外把髋 z / 横杆等效阻尼调到 `220 / 60 N·s/m`。演示仍用近距斜侧跟随镜头。
