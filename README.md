...
100 km/h 目标把左右轮距从 227 mm 加到 4 倍（910 mm），并把高速机体甲板加宽到 876 mm，用侧翼和贯穿轴梁贴齐髋座，而不是只把轮子撑开、机体仍是窄躯干。赛道改为 Isaac/ANYmal 式二维分形粗糙度，加上 10–15 cm、半宽约 2.5–3.6 m 的高斯 whoops 和稀疏土堆（崎岖段 RMS 约 78 mm，峰值约 197 mm）；路肩外侧再放 Unitree 风格的圆木/土堆只用于轮廓和侧光阴影，不进入 4× 轮距。取消了原先 30 mm 纵向挤出长波和 1.55 m 搓衣板——前者在近距镜头里看起来像平地，后者在 100 km/h 下是约 18 Hz 的俯仰锤击。
...
100 km/h 用速度 PI + 俯仰 PD（`pitch_kd=36`）、0.70 m/s² 加速，横杆位置伺服降到 4000 N/m 让菱形能轻微让步。4× 宽机体在可见 whoops 上跑完 64 s：峰值 28.15 m/s（101.3 km/h），驶过 1066 m，末段 y≈−1.9 m，最大俯仰 5.1°、最大横滚 2.6°。这是崎岖段上的完整巡航，不是只在平地垫上摸到 100 km/h。1x–3.5x 轮距仍无法同时把横滚压住并跑到该速度。演示用近距斜侧跟随镜头和侧光阴影。
