...
100 km/h 目标把左右轮距从 227 mm 加到 4 倍（910 mm），并把高速机体甲板加宽到 876 mm，用侧翼和贯穿轴梁贴齐髋座，而不是只把轮子撑开、机体仍是窄躯干。赛道改为 Isaac/ANYmal 式二维分形粗糙度，加上越野 whoops（约 7–12 cm、长度 2.7–3.9 m 的余弦凸起）和稀疏土堆；路肩外侧再放 Unitree 风格的圆木/土堆只用于轮廓和侧光阴影，不进入 4× 轮距。取消了原先 1.55 m 搓衣板——那在 100 km/h 下是约 18 Hz 的俯仰锤击，看起来也像一条挤出来的长波。

四连杆长度与轮距是互补的，但不要绑在一起加长：轮距管横滚杠杆，连杆长度管行程。当前 190 mm 连杆在 348 mm 站立菱形下已有约 174 mm 压缩行程；若只把连杆加到 230 mm 却保持同一站立高度、又不加长横杆，菱形更折叠，行程反而变短。正确做法是保持站立重心 0.408 m，让横杆在高速时以较低位置增益当悬架，而不是把机器人垫高。

100 km/h 用速度 PI + 俯仰 PD（`pitch_kd=32`）、0.70 m/s² 加速，并把横杆位置伺服从 8000 N/m 降到 1400 N/m，让菱形能吞 whoops。演示仍用近距斜侧跟随镜头，侧光阴影让起伏可读。峰值与是否跑完 64 s 以 `uv run python scripts/verify_high_speed.py` 的最新输出为准：这是崎岖段上的峰值速度，不是默认的全程巡航。
