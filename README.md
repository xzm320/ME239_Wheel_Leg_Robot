...
100 km/h 目标用速度 PI + 俯仰 PD，在 3000 m 赛道上以 0.70 m/s² 从静止加速。峰值约 26.25 m/s（94.5 km/h），行驶约 686 m 后进入崎岖段；随后因横移冲出 6 m 路面、机身高度塌缩而失败，不是完整的 100 km/h 巡航。下蹲或下移机身质心、以及把髋座/横杆/四连杆铰链阻尼调高，都会在 65 km/h 附近提前诱发 15° 横滚，因此 100 km/h 尝试仍使用 XML 默认柔顺（髋 x `100 N·s/m`、髋 z `180 N·s/m`、横杆等效 `50 N·s/m`）和站立高度 0.408 m。演示镜头改回联合控制的近距斜侧跟随（距离 3 m、方位 116°、俯角 −20°），便于看清车身俯仰和轮腿运动。
