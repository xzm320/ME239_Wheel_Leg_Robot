...
6. 建立控制器、参数辨识和高速运行调参/评估流程（进行中：中等崎岖地形联合控制完成）
...
柔顺参数采用固定随机种子的 53 组候选进行有界多目标搜索。部署的圆整值为髋座 x 方向 `7100 N/m, 48 N·s/m`、z 方向 `7800 N/m, 165 N·s/m`、整根横杆等效 `900 N/m, 50 N·s/m`。相对上一组参数，垂向加速度 RMS 下降约 45%，95 分位下降约 71%，横杆 95 分位作用力下降约 36%，重心高度标准差仅由 10.94 mm 变为 11.00 mm。

下一步将针对 `hard` / `extreme` 地形加入接触状态与冲击指标，并评估轮毂电机和横杆执行器的连续功率、峰值载荷及热限制。
